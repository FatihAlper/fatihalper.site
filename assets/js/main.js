...
function initLanguagePreference() {
  ...
  function remember(locale) {
    ...
    try {
      window.localStorage.setItem('fa_locale', locale);
    } catch (error) {
      // Storage can be unavailable in strict privacy modes; the cookie still works.
    }

    const isSecure = window.location.protocol === 'https:';
    document.cookie = `fa_locale=${locale}; Max-Age=15552000; Path=/; SameSite=Lax${isSecure ? '; Secure' : ''}`;
  }
  ...
  if (switcher) {
    switcher.querySelectorAll('a[lang]').forEach(link => {
      link.addEventListener('click', () => remember(link.getAttribute('lang')));
    });
  }
  ...
  let storedLocale = '';
  try {
    storedLocale = window.localStorage.getItem('fa_locale') || '';
  } catch (error) {
    storedLocale = '';
  }

  if (!supportedLocales.includes(storedLocale) || storedLocale === htmlLocale) return;

  params.set('lang', storedLocale);
...
function initManifestoStrip() {
  ...
  window.setInterval(() => show(index + 1), Number.isFinite(interval) ? interval : 6500);
}
